...
export const KEY_METRICS = [
  ...
  // ⚠️ 下面这个**不在官方关键指标看板的 6 项里**，是为「三端投放日报」补的 SOP 口径：
  // IG授权 = Ins授权回调·授权成功（数据口径文档 §5.2 的 T1「INS 人数」，SOP 用它）。
  // 它排在 IG绑定(task_id=110) 之前：授权成功 274 → 绑定完成 63（7/31 全量）。
  // 别和 ins_auth_click(pwa_ins_login_button_click，浮层点击 360) 混——那个只是点了按钮。
  { key: "ig_auth",         label: "IG授权",      event: "pwa_earning_ins_task_page_two_click", indicator: "event_users" },
];
