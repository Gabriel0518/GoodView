...
const METRICS = [
  ...
  // conversion = XMP 报表的「转化数」，媒体侧回传口径（Google/TikTok 自己算的转化）。
  // ⚠️ 它**不等于安装**：对 Savvy 实测 08-15 conversion=45 而真实安装 287（差 6 倍），
  //    因为各账户的优化目标不同，conversion 记的是各自的优化事件。安装要用 mobile_app_install。
  { id: "conversion",               zh: ["转化", "转化数", "转化次数"] },
  // mobile_app_install = MMP 口径的应用安装数。**上架包(Savvy/SmartReply)的安装用它**。
  // 2026-08-16 实测：Savvy 08-15=287(CPI $4.09)、SmartReply 08-15=497(CPI $0.98)，量级合理。
  // （早期注释说「MMP 指标对上架包全为 0」——那是当时只测了 SmartReply 的旧账户，对 Savvy 不成立，
  //   而且 SmartReply 现在也有数了。别再据此认为这个字段没用。）
  { id: "mobile_app_install",       zh: ["安装", "安装数", "应用安装"] },
  { id: "cpm",                      zh: ["千次展示成本"] },
  { id: "ctr",                      zh: ["点击率"] },
  { id: "cpc",                      zh: ["单次点击成本"] },
