...
//   account/campaign = 抓取范围（进 campaign_daily 的白名单）；metric = 抓哪些字段。
//   pwa_board = PWA(非公会)看板汇总账户集 —— 只影响看板口径，与抓取范围解耦（见 feishu-tables resolveDerivedGroups）。
export const CATEGORIES = { account: "广告账户", campaign: "广告系列", metric: "指标", pwa_board: "PWA看板账户" };
export const CATEGORY_OPTIONS = [CATEGORIES.account, CATEGORIES.campaign, CATEGORIES.metric, CATEGORIES.pwa_board];
...
export function validateConfigRow({ category, value, layer }) {
  ...
  if (cat === CATEGORIES.pwa_board) return { ok: true, kind: "pwa_board", value: v };
