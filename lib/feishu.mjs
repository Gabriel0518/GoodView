...
// 更新已存在字段（如给单选补选项）。field: 完整定义 { field_name, type, property? }。
export async function updateField(tableId, fieldId, field) {
  const data = await api("PUT", `/bitable/v1/apps/${app()}/tables/${tableId}/fields/${fieldId}`, field);
  return data.field;
}

// 删除字段（口径收窄、列不再产出时用）。⚠️ 连同该列数据一起删，不可逆——只对确认要撤的空列用。
export async function deleteField(tableId, fieldId) {
  return api("DELETE", `/bitable/v1/apps/${app()}/tables/${tableId}/fields/${fieldId}`);
}
...
// —— 视图：列出 / 设置排序 ——
// 为什么需要：增量窗口同步每轮把「本窗口」的行删掉再追加到表尾，飞书记录的物理顺序 =
// 历次追加的拼接结果，必然乱序（老历史在前、每轮新窗口跟在后面）。靠插入顺序保证展示顺序
// 是做不到的 —— 只能给视图挂一条持久排序规则，之后新灌的记录自动归位。
export async function listViews(tableId) {
  const data = await api("GET", `/bitable/v1/apps/${app()}/tables/${tableId}/views?page_size=100`);
  return data.items || [];
}

// ⚠️ 排序走的是 base/v3（不是 bitable/v1）：bitable/v1 的「更新视图」只认 filter/hidden_fields，没有排序。
// sortConfig: [{ field: "日期", desc: true }]（field 用字段名），传 [] 清空排序。
export async function setViewSort(tableId, viewId, sortConfig) {
  return api("PUT", `/base/v3/bases/${app()}/tables/${tableId}/views/${viewId}/sort`, { sort_config: sortConfig });
}
...
// —— 记录：统计总行数（低成本：page_size=1，只读响应里的 total）——
// 增量同步下表会逐日累积，需要在逼近飞书单表上限（本 tenant 2 万行）前预警/裁剪。
export async function countRecords(tableId) {
  const data = await api("POST", `/bitable/v1/apps/${app()}/tables/${tableId}/records/search?page_size=1`, { field_names: [] });
  return Number(data.total ?? 0);
}

// —— 记录：取最旧的 n 行 record_id（按 date_num 升序）——
// 只在真的顶到上限、必须腾地方时才调用；会全表扫一次 date_num 列（比全字段轻）。
export async function oldestRecordIds(tableId, field, n) {
  if (n <= 0) return [];
  const recs = await listRecords(tableId, [field]);
  return recs
    .map((r) => ({ id: r.record_id, k: Number(cellStr(r.fields?.[field])) || 0 }))
    .sort((a, b) => a.k - b.k)
    .slice(0, n)
    .map((r) => r.id);
}
...
// fieldNames 可选：给定则只取这些列（省流量）。
// viewId 可选：给定则**按该视图的排序/筛选**返回（不给 = 按记录物理顺序，跟界面看到的顺序不一致）。
// 返回 [{ record_id, fields }]。
export async function listRecords(tableId, fieldNames, viewId) {
  const out = [];
  let pageToken = "";
  do {
    const qs = new URLSearchParams({ page_size: "500", ...(pageToken ? { page_token: pageToken } : {}) });
    const body = {
      ...(fieldNames && fieldNames.length ? { field_names: fieldNames } : {}),
