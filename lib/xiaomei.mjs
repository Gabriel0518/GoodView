...
// ═══════════ Savvy 在 af_events 里的 app_id（跨端自动发现）═══════════
// 【为什么不能写死】AF 对安卓用包名当 app_id（com.gigpulse.savvy），对 iOS 用 **id<AppStoreID>**
// —— 两者毫无关系。用户 2026-08-20 把 Savvy iOS 的 Push 端点配成和安卓同一个 URL（端点本身不做
// app_id 白名单，见 app/api/af/push），所以第一条 iOS 事件会带着一个我们从没见过的 app_id 进来。
// 任何写死 'com.gigpulse.savvy' 的地方都会**静默漏掉整个 iOS 端**，且不会报错。
//
// 【判定规则】env 显式列出的 ∪ 事件里 campaign 名含 savvy 的 app_id。
// Savvy 的投放系列名一律带 Savvy 前缀（Savvy-IOS-wei-install-0819 / Savvy_syh_260814_USA_1_register
// / Savvy-小美-wei-0816…），而 SmartReply 的是 Smart Reply-*，两者不会互相命中。
// 这和 savvy-account-scan.mjs 认 XMP 账户用的是同一条线索（账户名不可靠、系列名可靠）。
//
// ⚠️ 别用 raw 里的 app_name / platform 认产品：Savvy 安卓的回传**这两个字段整列为空**
//    （2026-08-20 实测：25257 条全 null），只有 SmartReply 和 AF 的测试报文才有。
export const SAVVY_AF_CAMPAIGN_PATTERN = "savvy";

export async function resolveSavvyAfAppIds(query) {
  const fromEnv = (process.env.SAVVY_AF_APP_ID || "com.gigpulse.savvy")
    .split(",").map((x) => x.trim()).filter(Boolean);
  try {
    const { rows } = await query(
      `SELECT DISTINCT app_id FROM af_events
        WHERE campaign ~* $1 AND app_id IS NOT NULL AND app_id <> ''`,
      [SAVVY_AF_CAMPAIGN_PATTERN],
    );
    return [...new Set([...fromEnv, ...rows.map((r) => r.app_id)])];
  } catch {
    return fromEnv; // 库读不到就退回 env，别让报表整个挂掉
  }
}


// ═══════════ AF 事件里怎么取「业务用户 ID」和「端」（跨端，2026-08-20 实测）═══════════
// 【坑】iOS SDK 和安卓 SDK 的 event_value 结构**不一样**：
//   安卓 Savvy：{"user_id": 8757073, "af_device_id": "..."}                    ← user_id 在**顶层**
//   iOS  Savvy：{"params": {"user_id": 8751362, …}, "eventName": "…", "af_device_id": "…"}
//                                                                              ← user_id 在 **params 里**
//   实测：安卓 25284 条**没有一条**带 params 包装；iOS 顶层**一条都没有** user_id。
//   只写 event_value->>'user_id' 会让整个 iOS 端的用户 ID 全取成空，而且不报错。
// af_device_id 两端都在顶层，可以直接取。
// AF 顶层的 appsflyer_id / customer_user_id 对这两个 app 全为空（APP 端没调 setCustomerUserId）。
export const AF_USER_ID_SQL = `COALESCE(event_value->>'user_id', event_value->'params'->>'user_id')`;
export const AF_DEVICE_SQL = `event_value->>'af_device_id'`;
// 去重身份：优先业务 user_id，退到设备，再退到报文指纹（保证不会把多个人算成一个）
export const AF_IDENTITY_SQL =
  `COALESCE(appsflyer_id, customer_user_id, ${AF_USER_ID_SQL}, ${AF_DEVICE_SQL}, dedupe_key)`;

// 端。⚠️ 不能靠 platform / idfa / idfv / advertising_id：
//   Savvy 两端都不回传 platform；iOS 首批 7 条事件 idfa/idfv/advertising_id **全为空**。
//   AF 对 iOS app 一律用 id<AppStoreID> 当 app_id（如 id6787070836），这个最可靠。
export const AF_PLATFORM_SQL = `CASE WHEN app_id ~ '^id[0-9]+$' THEN 'ios'
       WHEN COALESCE(NULLIF(platform,''),'') <> '' THEN platform
