...
// 【PWA 为什么也走 DMS】(2026-08-16 验证 + 用户拍板)
//   所有产品的后端共用同一套 PWA 埋点、进同一个 BytePlus app → BytePlus 的「全量−AI公会」**是多产品
//   集合**，不是 PWA 本体。用事件属性 is_apk 拆开可以看得很清楚（注册，America/Chicago 日）：
//     08-13  非APK 181 / APK 155   ← PWA 还在投
//     08-14  非APK  88 / APK 208   ← PWA 停投当天，非APK 腰斩；APK 反涨（Savvy 同日从 17 放量到 116）
//     08-15  非APK  84 / APK 230
//   而非APK 那部分(88/84) 与业务库 PWA 本体(app_name=3) 的 82/72 高度吻合 → 证实混入。
//   故 注册/IG绑定/成材 改走业务库（与「关键指标日报」早已采用的口径一致，也与 Savvy 同源可比）。
//   ⚠️ GoLive 业务库没有（pwa_distribution 未接）→ 只能继续用 BytePlus，**它仍是多产品集合**，
//      看板说明里已标注。
