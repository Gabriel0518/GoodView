...
export async function fetchFunnelUsers({ steps, period, profileExpressions = [], windowDays = 1, timezone }) {
  ...
  const s = d0.data_item_list?.[0]?.summary_list || [];
  if (!s.length) return steps.map(() => 0); // 该窗口没人进第一步
  return [Number(s[2]) || 0, ...s.slice(3).map((x) => Number(Array.isArray(x) ? x[0] : x) || 0)];
}
