...
// 事件配置弹窗：从浮动侧栏打开（深链 ?modal=events）。复用 BoardModal 外壳模式。表格更宽 → max-w-5xl。
// 注：不监听 Esc 关闭——内层 EventEditor(z-50) 打开时 Esc 会误关整个弹窗、丢草稿。用 X / 背景点击关闭。
...
export function EventsModal({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-strong/40 p-3 sm:p-6" onPointerDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="mx-auto flex h-full w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-border bg-canvas shadow-md">
        <header className="flex items-center justify-between gap-3 border-b border-border bg-surface px-5 py-3">
          <h1 className="text-[15px] font-semibold text-strong">事件配置</h1>
